...
| TYPE            | 128 BIT | 192 BIT | 256 BIT | DESCRIPTION                                                                                                                 |
| --------------- | ------- | ------- | ------- | --------------------------------------------------------------------------------------------------------------------------- |
| ECB             | X       | X       | X       | 简单、最原始的模式。它将明文分成独立的块，每一块独立加密;明文块 A$\rightarrow$密钥 **$\rightarrow$** 密文块 A     |
| CBC             | X       | X       | X       | 每一个明文块在加密前，先与前一个密文块进行**XOR（异或）** 运算。第一个块则与 **IV** 异或。                      |
| CFB1            | X       | X       | X       | 它们加密的是 IV 和前一个密文，然后与明文进行异或；明文不需要填充（Padding）到 16 字节的倍数，<br />数据是多长，密文就是多长 |
| CFB8            | X       | X       | X       |                                                                                                                             |
| CFB128          | X       | X       | X       |                                                                                                                             |
| OFB             | X       | X       | X       |                                                                                                                             |
| CTR             | X       | X       | X       | 它加密的是一个“计数器”（通常由 IV/Nonce + 序号组成），然后将加密后的结果与明文进行异或（XOR）                             |
| CCM             | X       | X       | X       |                                                                                                                             |
| GCM             | X       | X       | X       | 基于计数器模式（CTR），并增加了一个极其重要的功能：**身份认证（Authentication）** 。                                  |
| XTS             | X       |         | X       | 专门为扇区存储设计。在磁盘上，数据的物理位置非常重要                                                                        |
| WRAP            | X       | X       | X       | 在密钥管理系统中，当你需要把一个“数据加密密钥 (DEK)”发送给别人时，<br />需要用一个“密钥加密密钥 (KEK)”把它包起来        |
| WRAP_PAD        | X       | X       | X       |                                                                                                                             |
| OCB             | X       | X       | X       | 一种性能极高的 AEAD 模式；在加密过程中同时进行认证，只需扫一遍数据即可完成。                                                |
| CBC_HAMC_SHA1   | X       |         | X       | 一种**混合方案** ，将加密（CBC）和认证（HMAC）组合在一起，<br />实现“认证加密”（Authenticated Encryption）。        |
| CBC_HMAC_SHA256 | X       |         | X       |                                                                                                                             |
